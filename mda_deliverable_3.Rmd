...
apt_buildings_sst %>% mutate(age_of_building = factor(age_of_building)) %>% mutate(age_of_building = fct_reorder(age_of_building, no_of_storeys, mean)) %>% ggplot(aes(age_of_building, no_of_storeys)) + geom_boxplot() + scale_y_log10()

```

I chose to reorder the factor `age_of_building` because the plot before the ordering is hard to understand; the number of storeys in each `age_of_building` group is hard to read. So, I had to reorder the `age_of_building` values by the average number of storeys in each group. 

The resultant plot is much easier to read; at a glance, one can easily see the minimum, maximum, and median number of storeys.
<!----------------------------------------------------------------------------->
...
(collapse_to_other <- apt_buildings_sst %>% 
  ...
  geom_boxplot(aes(colour = age_of_building), show.legend = FALSE) + scale_y_log10())

```

I grouped `ancient` and `very old` buildings together because I am only interested in investigating `old`, `recent`, and `more recent` buildings individually. `ancient` and ` very old` are just too back in years that I decided to group them under `other`.
<!----------------------------------------------------------------------------->
...
apt_buildings_sst_fltr %>% ggplot(aes(year_built, no_of_storeys)) + geom_point() + geom_smooth(method = "lm")
