...
To explore each of the four chosen datasets, I will look at class of the data, variable types, number of rows and columns, the variable names, the total number of missing values (NAs). The code below are from what was taught in STAT545A and the R for Data Science book by Hadley Wickham & Garrett Grolemund
...
First, I chose **apt_buildings** because when I explored it, I could see that there are `r ncol(apt_buildings)` columns in the dataset with `year_built` and `no_of_storeys` as part of the variables. So, I became curious to see if the number of storeys built changed as the years rolled by.

Second, I chose **flow_sample** because I wanted to visualize the months with the highest flow. To find out, I ran the code below.
```{r}
# visualize months with highest flow
flow_sample %>% ggplot(aes(month, flow)) + 
...
+ `apt_buildings`: Does the number of storeys built change over the years?
+ `flow_sample`: Which months have the highest flow intensity?
...
**Final choice**: Out of the four research questions, it seems investigating whether there's a change in the number of storeys built as the years roll by would be the most interesting.Therefore, I choose the `apt_buildings` dataset.
